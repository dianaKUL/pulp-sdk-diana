...
class Runner(runner.Runner):
    ...
    def __get_rtl_path(self):
        ...
        if self.rtl_path is None:
            ...
            self.__create_symlink(self.rtl_path, 'xcelium.d')
    ...
    def __get_sim_cmd(self):
        ...
        if simulator == 'xcelium':
            ...
            xmsim_args.append('-sv_lib %s/../rtl/tb/remote_bitbang/librbs.so' % (os.environ.get('XCSIM_PATH')))
            ...
            xmelab_cmd = "%s worklib.tb_pulp:sv %s" % (cds_xmelab, ' '.join(xmelab_args))
            xmsim_cmd = "%s tb_pulp:sv %s" % (cds_xmsim, ' '.join(xmsim_args))
